	...
	if (schema.overwrite) {
		...
		if (!confirm) {
			const response = await prompts({
				type: "confirm",
				name: "confirm",
				message: `The file ${schema.fileName
		...
		if (confirm) {
			...
			logger.success(
				`🚀 Schema was ${schema.overwrite ? "overwritten" : "appended"
				} successfully!`,
			);
...
export const generate = new Command("generate")
	...
	.option(
		"-cfe, --cf-env <environment>",
		"the target cloudflare environment (define in you wrangler config), default to root one ('').",
		"",
	)
	.option(
		"--config <config>",
