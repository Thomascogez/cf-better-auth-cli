...
const findClosestNodeModulesFolder = (cwd: string, maxDepth = 10) => {
	let currentDir = cwd;
	for (let i = 0; i < maxDepth; i++) {
		const nodeModulesFolder = path.join(currentDir, "node_modules");
...
const ensureTempFolderExists = (cwd: string) => {
	const tempFolder = path.join(cwd, ".cf-better-auth-cli");
...
export async function getConfig({
	...
	const nodeModulesFolder = findClosestNodeModulesFolder(cwd);
	if (!nodeModulesFolder) {
		throw new BetterAuthError("Could not find node_modules folder")
	}

	const wranglerPlatformInjectorScriptPath = path.join(nodeModulesFolder, ".cf-better-auth-cli", "env-injector.mjs");
	const stubCloudflareWorkerModulePath = path.join(nodeModulesFolder, ".cf-better-auth-cli", "stub-cloudflare-worker-module.mjs");

	ensureTempFolderExists(nodeModulesFolder);

	await writeFile(wranglerPlatformInjectorScriptPath, `
		export default (cloudflarePlatform) => { globalThis.cloudflarePlatform = cloudflarePlatform; }
	`);

	await writeFile(stubCloudflareWorkerModulePath, `
		export default { env: globalThis.cloudflarePlatform?.env, cf: globalThis.cloudflarePlatform?.cf, ctx: globalThis.cloudflarePlatform?.ctx, caches: globalThis.cloudflarePlatform?.caches };
	`)

	let wranglerPlatformProxy: Awaited<ReturnType<typeof getPlatformProxy>> | null = null;
	try {
		wranglerPlatformProxy = await getPlatformProxy({
			environment: cfEnv
		})
		const jiti = createJiti(import.meta.url, { ...jitiOptions(cwd), alias: { "cloudflare:workers": stubCloudflareWorkerModulePath } });

		const injectWranglerPlatformFN = await jiti.import<(platform: Awaited<ReturnType<typeof getPlatformProxy>>) => void>(wranglerPlatformInjectorScriptPath, { default: true });
		injectWranglerPlatformFN(wranglerPlatformProxy);
		...
		if (wranglerPlatformProxy) {
			await wranglerPlatformProxy.dispose();
		}
	}
}
